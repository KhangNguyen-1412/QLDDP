...
import {
  ...
  getDownloadURL,
} from "firebase/storage"; // Thêm imports cho Firebase Storage
...
function App() {
  ...
  const handleUploadQrCode = async () => {
    ...
    try {
      const response = await axios.post(
        CLOUDINARY_API_URL_IMAGE_UPLOAD,
        formData
      );
  ...
  const [currentElectricityReading, setCurrentElectricityReading] =
    useState("");
  ...
  const [totalCalculatedDaysAllResidents, setTotalCalculatedDaysAllResidents] =
    useState(0);
  ...
  const [selectedResidentForReminder, setSelectedResidentForReminder] =
    useState("");
  ...
  const [selectedCostSharingDetails, setSelectedCostSharingDetails] =
    useState(null);
  ...
  const [selectedResidentForCleaning, setSelectedResidentForCleaning] =
    useState("");
  ...
  const [newPendingResidentType, setNewPendingResidentType] =
    useState("pending"); // Loại thành viên chờ: 'pending' hoặc 'invited
  ...
  const handleAddPendingResident = async () => {
    ...
    const pendingResidentsRef = collection(
      db,
      `artifacts/${currentAppId}/public/data/pendingResidents`
    );
    try {
      await addDoc(pendingResidentsRef, {
        ...
        type: newPendingResidentType, // Lưu lại loại thành viên
        createdAt: serverTimestamp(),
      });
      setNewPendingResidentName("");
      alert(
        `Đã thêm thành viên "${newPendingResidentName.trim()}" vào danh sách thành công!`
      );
    } catch (error) {
      console.error("Lỗi khi thêm thành viên đặc biệt:", error);
      alert("Đã xảy ra lỗi.");
    }
  };
  ...
  const handleMoveInPendingResident = async (pendingResident) => {
    const activeResidentsCount = residents.filter(
      (res) => res.status !== "inactive"
    ).length;
    if (activeResidentsCount >= 8) {
      alert(
        "Phòng đã đủ 8 người. Vui lòng hoàn tất thủ tục cho một thành viên cũ trước khi chuyển thành viên mới vào."
      );
      return;
    }
    ...
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn chuyển "${pendingResident.name}" vào danh sách thành viên chính thức?`
      )
    ) {
      return;
    }
  ...
  const handleSendFeedback = async (e) => {
    ...
    const feedbackCollectionRef = collection(
      db,
      `artifacts/${currentAppId}/public/data/feedback`
    );
    try {
  ...
  useEffect(() => {
    if (db && userRole === "admin") {
      const feedbackQuery = query(
        collection(db, `artifacts/${currentAppId}/public/data/feedback`),
  ...
  const handleUpgradeToAdmin = async (targetUserId) => {
    ...
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn nâng cấp người này lên vai trò quản trị viên không?`
      )
    ) {
      return;
    }
  ...
  const [newFormerResidentStudentId, setNewFormerResidentStudentId] =
    useState("");
  const [newFormerResidentBirthday, setNewFormerResidentBirthday] =
    useState("");
  const [newFormerResidentDormEntryDate, setNewFormerResidentDormEntryDate] =
    useState("");
  const [newFormerResidentAcademicLevel, setNewFormerResidentAcademicLevel] =
    useState("");
  const [
    newFormerResidentDeactivatedDate,
    setNewFormerResidentDeactivatedDate,
  ] = useState(""); // Ngày vô hiệu hóa thủ công
  const [showAddFormerResidentModal, setShowAddFormerResidentModal] =
    useState(false);
  ...
  const [selectedNotificationDetails, setSelectedNotificationDetails] =
    useState(null); // Để hiển thị chi tiết thông báo
  ...
  const handleToggleAttendancePermission = async (
    targetUserId,
    currentStatus
  ) => {
    if (userRole !== "admin") {
    ...
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn ${
          currentStatus ? "thu hồi" : "trao"
        } quyền điểm danh cho người này?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      alert(
        `Đã ${!currentStatus ? "trao" : "thu hồi"} quyền điểm danh thành công!`
      );
    } catch (error) {
  ...
  const handleAddMemory = async (e) => {
    ...
    if (
      ...
      !newMemoryEventName ||
      !newMemoryPhotoDate ||
      newMemoryImageFile.length === 0
    ) {
      setMemoryError(
        "Vui lòng điền đầy đủ thông tin và chọn ít nhất một file."
      );
      return;
    }
    if (!userId) {
    ...
    try {
      ...
      for (const file of newMemoryImageFile) {
        ...
        const response = await axios.post(
          CLOUDINARY_API_URL_AUTO_UPLOAD,
          formData,
          {
            headers: {
      ...
      const memoriesCollectionRef = collection(
        db,
        `artifacts/${currentAppId}/public/data/memories`
      );
      const uploaderStudentId =
        loggedInResidentProfile?.studentId ||
        allUsersData.find((u) => u.id === userId)?.studentId;
      await addDoc(memoriesCollectionRef, {
        ...
        uploadedByName: loggedInResidentProfile
          ...
          : allUsersData.find((u) => u.id === userId)?.fullName ||
            "Người dùng ẩn danh",
  ...
  const handleUpdateMemory = async (e) => {
    ...
    try {
      ...
      let updatedFilesInfo = editingMemory.files
        ? [...editingMemory.files]
        : [];
  ...
  useEffect(() => {
    ...
    try {
      if (!firebaseConfig.apiKey || !firebaseConfig.projectId) {
        console.error(
          "Cấu hình Firebase bị thiếu hoặc rỗng. Vui lòng kiểm tra lại cấu hình."
        );
        setIsAuthReady(true);
        return;
      }
      ...
      console.log(
        "DEBUG INIT: storage object after setStorage:",
        firebaseStorage
      ); // Log để kiểm tra

      console.log(
        "6. setDb, setAuth và setStorage đã được gọi. Đang chờ onAuthStateChanged..."
      );

      const unsubscribe = onAuthStateChanged(firebaseAuth, async (user) => {
        console.log(
          "6. onAuthStateChanged đã được kích hoạt. Người dùng hiện tại:",
          user ? user.uid : "null"
        );
        if (user) {
          ...
          if (!user.emailVerified) {
            ...
            setAuthError(
              "Tài khoản của bạn chưa được xác minh email. Vui lòng kiểm tra hộp thư đến và nhấp vào liên kết xác minh. Bạn cũng có thể sử dụng nút 'Gửi lại email xác minh' bên dưới."
            );
            setIsAuthReady(true);
            return;
          }
          ...
          const residentsCollectionRef = collection(
            firestoreDb,
            `artifacts/${currentAppId}/public/data/residents`
          );
          ...
          if (linkedResidentId) {
            const residentDoc = await getDoc(
              doc(residentsCollectionRef, linkedResidentId)
            );
            if (residentDoc.exists()) {
              currentLoggedInResidentProfile = {
                id: residentDoc.id,
                ...residentDoc.data(),
              };
              console.log(
                "9. Hồ sơ cư dân được liên kết (từ linkedResidentId):",
                currentLoggedInResidentProfile.name
              );
            } else {
              console.log(
                "9. linkedResidentId trong tài liệu người dùng không hợp lệ hoặc cư dân không tồn tại."
              );
          ...
          if (!currentLoggedInResidentProfile && fetchedFullName) {
            const qResidentByName = query(
              residentsCollectionRef,
              where("name", "==", fetchedFullName)
            );
            ...
            if (!residentSnapByName.empty) {
              ...
              if (
                !matchedResident.data().linkedUserId ||
                matchedResident.data().linkedUserId === user.uid
              ) {
                await updateDoc(userDocRef, {
                  linkedResidentId: matchedResident.id,
                });
                currentLoggedInResidentProfile = {
                ...
                console.log(
                  "9. Đã tìm và liên kết hồ sơ cư dân theo tên:",
                  currentLoggedInResidentProfile.name
                );
              } else {
                console.log(
                  `Cư dân "${fetchedFullName}" đã được liên kết với một người dùng khác.`
                );
              }
            } else {
              console.log(
                `Không tìm thấy hồ sơ cư dân có tên "${fetchedFullName}". Admin có thể cần thêm/liên kết thủ công.`
              );
            }
          }
          ...
          console.log(
            `DEBUG AUTH: User ID: ${
              user.uid
            }, User Role: ${fetchedRole}, Linked Resident: ${
              currentLoggedInResidentProfile
                ? currentLoggedInResidentProfile.name
                : "None"
            }`
      ...
      console.error(
        "Lỗi nghiêm trọng khi khởi tạo Firebase (tổng thể):",
        error
      );
      setIsAuthReady(true);
    }
  ...
  const handleUpdateFundManually = async () => {
    ...
    if (costSharingHistory.length === 0) {
      setBillingError(
        "Chưa có lịch sử chia tiền nào để cập nhật. Vui lòng tính tiền và chia tiền ít nhất một lần."
      );
      return;
    }
  ...
  const handleAddFundExpense = async (e) => {
    ...
    if (costSharingHistory.length === 0) {
      setBillingError(
        "Chưa có lịch sử chia tiền nào để ghi nhận chi tiêu. Vui lòng tính tiền và chia tiền ít nhất một lần."
      );
      return;
    }
    ...
    const newRemainingFund =
      (latestCostSharingRecord.remainingFund || 0) - amount;

    const expensesCollectionRef = collection(
      db,
      `artifacts/${currentAppId}/public/data/fundExpenses`
    );
  ...
  useEffect(() => {
    ...
    const expensesQuery = query(
      collection(db, `artifacts/${currentAppId}/public/data/fundExpenses`),
  ...
  const [newFormerResidentAvatarFile, setNewFormerResidentAvatarFile] =
    useState(null);
  const [
    formerResidentAvatarUploadProgress,
    setFormerResidentAvatarUploadProgress,
  ] = useState(0);
  const [isUploadingFormerResidentAvatar, setIsUploadingFormerResidentAvatar] =
    useState(false);
  const [formerResidentAvatarError, setFormerResidentAvatarError] =
    useState("");
  const [
    uploadFormerResidentAvatarProgress,
    setUploadFormerResidentAvatarProgress,
  ] = useState(0);

  //Cho popup sửa tiền bối
  const [editingFormerResidentAvatarFile, setEditingFormerResidentAvatarFile] =
    useState(null);
  const [
    isUploadingEditingFormerResidentAvatar,
    setIsUploadingEditingFormerResidentAvatar,
  ] = useState(false);
  const [
    uploadEditingFormerResidentAvatarProgress,
    setUploadEditingFormerResidentAvatarProgress,
  ] = useState(0);
  ...
  const handleUploadFormerResidentAvatar = async (formerResidentId) => {
    ...
    if (
      ...
      !userId ||
      userRole !== "admin" ||
      !newFormerResidentAvatarFile
    ) {
      setFormerResidentAvatarError(
        "Bạn không có quyền hoặc vui lòng chọn một tệp ảnh."
      );
      return;
    }
    ...
    try {
      ...
      const downloadURL = response.data.secure_url;
      console.log(
        "Avatar tiền bối tải lên Cloudinary thành công, URL:",
        downloadURL
      );
      ...
      setFormerResidentAvatarUrls((prev) => ({
        ...prev,
        [formerResidentId]: downloadURL,
      }));
  ...
  useEffect(() => {
    if (!db || !isAuthReady || userId === null) {
      console.log(
        "Lắng nghe tiền bối: Đang chờ DB, Auth hoặc User ID sẵn sàng."
      );
      return;
    }
    console.log("Bắt đầu lắng nghe cập nhật thông tin tiền bối...");

    const formerResidentsCollectionRef = collection(
      db,
      `artifacts/${currentAppId}/public/data/formerResidents`
    );
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
          ...
          if (
            data.deactivatedAt &&
            typeof data.deactivatedAt.toDate === "function"
          ) {
  ...
  const handleSendCustomNotification = async (e) => {
    ...
    if (
      !db ||
      !userId ||
      (userRole !== "admin" && userId !== "BJHeKQkyE9VhWCpMfaONEf2N28H2")
    ) {
    ...
    if (
      newNotificationRecipient !== "all" &&
      !allUsersData.find((u) => u.id === newNotificationRecipient)
    ) {
  ...
  useEffect(() => {
    if (!db || !auth || !storage || !isAuthReady || userId === null) {
      console.log(
        "Lắng nghe kỷ niệm: Đang chờ DB, Auth hoặc User ID sẵn sàng."
      );
      return;
    }
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
          ...
          if (!data.files) {
            ...
            if (data.fileUrl) {
              ...
              data.files.push({
                ...
                fileType:
                  data.fileType ||
                  (data.fileUrl.match(/\.(mp4|mov|avi|wmv|flv)$/i)
                    ? "video"
                    : "image"), // Đoán loại nếu không có
              });
            }
          }
        ...
        setTotalPagesMemories(
          Math.ceil(allFetchedMemories.length / itemsPerPageMemories)
        );
        ...
        const memoriesForCurrentPage = allFetchedMemories.slice(
          startIndex,
          endIndex
        );
        ...
        if (
          currentPageMemories >
            Math.ceil(allFetchedMemories.length / itemsPerPageMemories) &&
          allFetchedMemories.length > 0
        ) {
          setCurrentPageMemories(1);
        } else if (allFetchedMemories.length === 0) {
          // Nếu không có kỷ niệm nào, đảm bảo currentPageMemories là 1
          setCurrentPageMemories(1);
        }

        setMemories(memoriesForCurrentPage); // Cập nhật state 'memories' chỉ với các bài của trang hiện tại
        console.log(
          "Đã cập nhật kỷ niệm phòng (có bộ lọc và phân trang):",
          memoriesForCurrentPage
        );
      },
      (error) => {
    ...
    userId,
    searchTermMemory,
    filterUploaderMemory,
    currentPageMemories,
    itemsPerPageMemories,
  ]); // THÊM currentPageMemories và itemsPerPageMemories vào dependencies
  ...
  const handleAdminCreateAccount = async () => {
    ...
    if (
      newAccountFullName.trim() === "" ||
      newAccountStudentId.trim() === "" ||
    ...
    const personalEmailToUse =
      newAccountPersonalEmail.trim() || `${studentIdToRegister}@dormapp.com`; // Email cá nhân hoặc email nội bộ nếu trống
    ...
    try {
      ...
      const qStudentId = query(
        usersRef,
        where("studentId", "==", studentIdToRegister)
      );
      const snapshotStudentId = await getDocs(qStudentId);
      if (!snapshotStudentId.empty) {
        setAuthError(
          "Mã số sinh viên này đã được đăng ký. Vui lòng sử dụng mã khác."
        );
        return;
      }
      ...
      await setDoc(
        doc(db, `artifacts/${currentAppId}/public/data/users`, user.uid),
        {
      ...
      alert(
        `Đã tạo tài khoản cho ${newAccountFullName.trim()} (MSSV: ${studentIdToRegister}) thành công! Người dùng cần đăng nhập và xác minh email cá nhân.`
      );
      ...
      if (error.code === "auth/email-already-in-use") {
        setAuthError(
          "Email này đã được sử dụng bởi một tài khoản khác. Vui lòng dùng email khác."
        );
      } else {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      const qStudentId = query(
        usersRef,
        where("studentId", "==", studentIdToLogin)
      );
      ...
      if (!personalEmail) {
        setAuthError(
          "Tài khoản không có email liên kết. Vui lòng liên hệ quản trị viên."
        );
        return;
      }
      ...
      const loginHistoryRef = collection(
        db,
        `artifacts/${currentAppId}/public/data/loginHistory`
      );
      ...
      console.log(
        "Đăng nhập thành công! Đang kiểm tra trạng thái xác minh email..."
      );
      ...
      if (
        ...
        // Thêm invalid-credential
        setAuthError("Mã số sinh viên hoặc mật khẩu không đúng.");
      } else if (error.code === "auth/invalid-email") {
        setAuthError(
          "Email liên kết không hợp lệ. Vui lòng liên hệ quản trị viên."
        );
  ...
  const handleRegister = async () => {
    ...
    if (
      fullName.trim() === "" ||
      newStudentIdForAuth.trim() === "" ||
      personalEmailForRegister.trim() === "" ||
      password.trim() === ""
    ) {
      setAuthError(
        "Vui lòng nhập đầy đủ Họ tên, Mã số sinh viên, Email và Mật khẩu."
      );
      return;
    }
    ...
    try {
      ...
      alert(
        `Đăng ký thành công! Vui lòng kiểm tra hộp thư tại ${personalEmail} để xác minh tài khoản trước khi đăng nhập.`
      );
  ...
  const handleResendVerificationEmail = async () => {
    ...
    try {
      await sendEmailVerification(user);
      setAuthError(
        "Đã gửi email xác minh thành công! Vui lòng kiểm tra hộp thư đến của bạn. Sau khi xác minh, hãy đăng nhập lại."
      );
    } catch (error) {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      setForgotPasswordMessage(
        "Đã gửi liên kết đặt lại mật khẩu đến email của bạn. Vui lòng kiểm tra hộp thư đến."
      );
    ...
    try {
      ...
      if (email.trim() !== currentUserData.email) {
        try {
          ...
          alert(
            "Email đã được thay đổi. Vui lòng kiểm tra email mới để xác minh lại tài khoản."
          );
        } catch (updateEmailError) {
          console.error("Lỗi khi cập nhật email trong Auth:", updateEmailError);
          setAuthError(
            `Lỗi khi cập nhật email: ${updateEmailError.message}. Vui lòng thử lại.`
          );
          return;
        }
      }
  ...
  const handleDeleteMemory = async (memoryId, files, uploadedByUserId) => {
    ...
    const isAllowedToDelete =
      userRole === "admin" || userId === uploadedByUserId;
    ...
    try {
      ...
      await deleteDoc(
        doc(db, `artifacts/${currentAppId}/public/data/memories`, memoryId)
      );
  ...
  const [editingCommonResidentData, setEditingCommonResidentData] =
    useState(null); // Lưu thông tin cư dân từ 'residents'
  const [
    editingCommonResidentUserLinkedData,
    setEditingCommonResidentUserLinkedData,
  ] = useState(null); // Lưu thông tin user liên kết
    ...
    try {
      ...
      if (editingCommonResidentUserLinkedData) {
        ...
        const userDocRef = doc(
          db,
          `artifacts/${currentAppId}/public/data/users`,
          editingCommonResidentUserLinkedData.id
        );
  ...
  const uploadAvatar = async (userId) => {
    ...
    const avatarStorageRef = ref(
      storage,
      `avatars/${userId}/${selectedAvatarFile.name}`
    );
  ...
  const handleMoveToFormerResidents = async () => {
    ...
    if (
      !window.confirm(
        "Bạn có chắc chắn muốn chuyển người này sang danh sách tiền bối không? Hành động này sẽ vô hiệu hóa tài khoản của họ."
      )
    ) {
      return;
    }
    ...
    const userToMove = allUsersData.find(
      (u) => u.linkedResidentId === residentId
    );
    ...
    try {
      ...
      alert(
        `Đã chuyển ${residentToMove.name} sang danh sách tiền bối thành công.`
      );
  ...
  const handleAddFormerResidentManually = async (e) => {
    ...
    if (
      !newFormerResidentName ||
      !newFormerResidentEmail ||
      !newFormerResidentDeactivatedDate
    ) {
    ...
    try {
      ...
      if (newFormerResidentAvatarFile) {
        ...
        try {
          ...
          console.error(
            "Lỗi khi tải ảnh avatar tiền bối lên Cloudinary:",
            uploadError
          );
          setFormerResidentAvatarError(
            `Lỗi khi tải ảnh avatar: ${uploadError.message}`
          );
          setIsUploadingFormerResidentAvatar(false);
  ...
  const handleDeleteFormerResident = async (residentId) => {
    ...
    if (
      !window.confirm("Bạn có chắc chắn muốn xóa thông tin tiền bối này không?")
    ) {
      return;
    }
  ...
  const createNotification = async (
    ...
    try {
      const notificationsCollectionRef = collection(
        db,
        `artifacts/${currentAppId}/public/data/notifications`
      );
  ...
  const markNotificationAsRead = async (notificationId) => {
    if (!db || !userId) {
      console.error(
        "DB hoặc User ID chưa sẵn sàng để đánh dấu thông báo đã đọc."
      );
      return;
    }
    try {
      ...
      setNotificationError(
        `Lỗi khi đánh dấu thông báo đã đọc: ${error.message}`
      );
    }
  };
  ...
  const [newNotificationRecipient, setNewNotificationRecipient] =
    useState("all"); // 'all' or a specific userId
  ...
  const [customNotificationSuccess, setCustomNotificationSuccess] =
    useState("");
  const [showAddNotificationModal, setShowAddNotificationModal] =
    useState(false);

  const [selectedMemoryForLightbox, setSelectedMemoryForLightbox] =
    useState(null); // Lưu toàn bộ đối tượng memory
  ...
  const [selectedResidentForAvatarUpload, setSelectedResidentForAvatarUpload] =
    useState(null);
  ...
  const handleUploadResidentAvatar = async (residentUserId) => {
    ...
    if (!residentUserId) {
      setAvatarUploadModalError(
        "Không tìm thấy ID người dùng để cập nhật avatar."
      );
      return;
    }
    if (userRole !== "admin") {
  ...
  const handleChangePassword = async () => {
    ...
    if (!auth || !userId) {
      setPasswordChangeMessage(
        "Hệ thống xác thực chưa sẵn sàng hoặc bạn chưa đăng nhập."
      );
      return;
    }
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "Lắng nghe điểm danh: Đang chờ DB, Auth, tháng hoặc User ID sẵn sàng."
      );
      return;
    }
    console.log(
      `Bắt đầu lắng nghe điểm danh hàng ngày cho tháng: ${selectedMonth}...`
    );

    const dailyPresenceCollectionRef = collection(
      db,
      `artifacts/${currentAppId}/public/data/dailyPresence`
    );
    let q;

    // Cả Admin và Member đã đăng nhập đều cần truy vấn tất cả bản ghi điểm danh để hiển thị
    if (userRole === "member" || userRole === "admin") {
      // Nếu là thành viên hoặc admin
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
          ...
          const endOfMonth = `${selectedMonth}-${String(lastDay).padStart(
            2,
            "0"
          )}`;
        ...
        console.log(
          `Đã cập nhật dữ liệu điểm danh cho tháng ${selectedMonth}:`,
          data
        );
      },
      (error) => {
    ...
    return () => {
      console.log(
        `Hủy đăng ký lắng nghe điểm danh hàng ngày cho tháng ${selectedMonth}.`
      );
      unsubscribe();
    };
  }, [
  ...
  ]); // Thêm loggedInResidentProfile vào dependency

  // Lấy các chỉ số đồng hồ được ghi nhận cuối cùng khi thành phần được gắn kết
  useEffect(() => {
    if (!db || !auth || !storage || !isAuthReady || userId === null) {
      console.log(
        "Lắng nghe chỉ số đồng hồ: Đang chờ DB, Auth hoặc User ID sẵn sàng."
      );
      return;
    }
    ...
    const meterReadingsDocRef = doc(
      db,
      `artifacts/${currentAppId}/public/data/meterReadings`,
      "currentReadings"
    );
  ...
  useEffect(() => {
    if (!db || !auth || !storage || !isAuthReady || userId === null) {
      console.log(
        "Lắng nghe lịch sử hóa đơn: Đang chờ DB, Auth hoặc User ID sẵn sàng."
      );
      return;
    }
    ...
    const billHistoryCollectionRef = collection(
      db,
      `artifacts/${currentAppId}/public/data/billHistory`
    );
  ...
  useEffect(() => {
    if (!db || !auth || !storage || !isAuthReady || userId === null) {
      console.log(
        "Lắng nghe lịch sử chia tiền: Đang chờ DB, Auth hoặc User ID sẵn sàng."
      );
      return;
    }
    ...
    const costSharingCollectionRef = collection(
      db,
      `artifacts/${currentAppId}/public/data/costSharingHistory`
    );
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
          ...
          if (
            data.calculatedDate &&
            typeof data.calculatedDate.toDate === "function"
          ) {
        ...
        history.sort(
          (a, b) => (b.calculatedDate || 0) - (a.calculatedDate || 0)
        );
  ...
  useEffect(() => {
    if (!db || !auth || !storage || !isAuthReady || userId === null) {
      console.log(
        "Lắng nghe lịch trực phòng: Đang chờ DB, Auth hoặc User ID sẵn sàng."
      );
      return;
    }
    ...
    const cleaningTasksCollectionRef = collection(
      db,
      `artifacts/${currentAppId}/public/data/cleaningTasks`
    );
  ...
  useEffect(() => {
    if (!db || !auth || !storage || !isAuthReady || userId === null) {
      console.log(
        "Lắng nghe gán kệ giày: Đang chờ DB, Auth hoặc User ID sẵn sàng."
      );
      return;
    }
    console.log("Bắt đầu lắng nghe gán kệ giày...");

    const shoeRackCollectionRef = collection(
      db,
  ...
  useEffect(() => {
    ...
    billHistory.forEach((bill) => {
      ...
      if (billDate) {
        const yearMonth = `${billDate.getFullYear()}-${String(
          billDate.getMonth() + 1
        ).padStart(2, "0")}`;
        if (!stats[yearMonth]) {
  ...
  useEffect(() => {
    if (!db || !isAuthReady || userId === null) {
      console.log(
        "Lắng nghe tất cả người dùng: Đang chờ DB, Auth hoặc User ID sẵn sàng."
      );
      return;
    }
  ...
  useEffect(() => {
    if (!db || !auth || !storage || !isAuthReady || userId === null) {
      console.log(
        "Lắng nghe thông báo: Đang chờ DB, Auth hoặc User ID sẵn sàng."
      );
      return;
    }
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        "Birthday notification check: Waiting for DB, Auth, User ID, allUsersData, or residents to be ready."
      );
      return;
    }
    ...
    const checkBirthdays = async () => {
      ...
      residents
        ...
        .forEach((resident) => {
          const userLinked = allUsersData.find(
            (u) => u.linkedResidentId === resident.id
          );
          if (userLinked && userLinked.birthday) {
            ...
            // Ví dụ: '1990-07-10'
            const [birthYearStr, birthMonthStr, birthDayStr] =
              userLinked.birthday.split("-");
            ...
            if (birthMonth === currentMonth && birthDay === currentDay) {
              ...
              if (!localStorage.getItem(notificationKey)) {
                ...
                const message = `🎉 Chúc mừng sinh nhật ${
                  userLinked.fullName || resident.name
                } tròn ${currentYear - parseInt(birthYearStr)} tuổi!`;
                ...
                console.log(
                  `Đã gửi thông báo sinh nhật cho ${userLinked.fullName} và lưu trạng thái vào localStorage.`
                );
              } else {
                console.log(
                  `Thông báo sinh nhật cho ${userLinked.fullName} (${currentYear}) đã được hiển thị rồi.`
                );
              }
            }
          }
  ...
  const handleAddResident = async () => {
    ...
    const activeResidentsCount = residents.filter(
      (res) => res.isActive !== false
    ).length;
    if (activeResidentsCount >= 8) {
      console.error(
        "Bạn chỉ có thể thêm tối đa 8 người đang hoạt động trong phòng."
      );
      setAuthError(
        "Bạn chỉ có thể thêm tối đa 8 người đang hoạt động trong phòng."
      );
  ...
  const handleUpdateFormerResident = async (e) => {
    ...
    if (
      !editingFormerResident.name ||
      !editingFormerResident.email ||
      !editingFormerResident.deactivatedAt
    ) {
      setAuthError("Vui lòng điền đầy đủ Họ tên, Email, Ngày ra khỏi phòng.");
      return;
    }
  ...
  const handleToggleResidentActiveStatus = async (
    residentId,
    residentName,
    currentStatus
  ) => {
    ...
    const actionText = currentStatus
      ? "bắt đầu quy trình rời phòng"
      : "khôi phục lại trạng thái hoạt động";
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn ${actionText} cho "${residentName}" không?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      await updateDoc(residentDocRef, {
        ...
        ...(newStatus === "pending_departure" && {
          departureRequestedAt: serverTimestamp(),
        }),
      });
  ...
  const handleToggleDailyPresence = async (residentId, day) => {
    ...
    if (
      userRole === "member" &&
      !memberCanTakeAttendance &&
      loggedInResidentProfile &&
      residentId !== loggedInResidentProfile.id
    ) {
    ...
    const currentStatus =
      monthlyAttendanceData[residentId]?.[String(day).padStart(2, "0")] || 0;
    ...
    try {
      ...
      console.log(
        ...
        } vào ngày ${day}/${selectedMonth} thành ${
          newStatus === 1 ? "Có ở" : "Không ở"
        }.`
      );
    } catch (error) {
  ...
  const calculateBill = async () => {
    ...
    if (isNaN(elecCurrent) || isNaN(waterCurrent)) {
      console.error(
        "Vui lòng nhập đầy đủ và chính xác các chỉ số điện nước hiện tại."
      );
      setBillingError(
        "Vui lòng nhập đầy đủ và chính xác các chỉ số điện nước hiện tại."
      );
    ...
    if (
      elecCurrent < lastElectricityReading ||
      waterCurrent < lastWaterReading
    ) {
      console.error(
        "Chỉ số hiện tại phải lớn hơn hoặc bằng chỉ số cuối cùng được ghi nhận."
      );
      setBillingError(
        "Chỉ số hiện tại phải lớn hơn hoặc bằng chỉ số cuối cùng được ghi nhận."
      );
    ...
    try {
      ...
      console.log(
        "Đã tính toán chi phí và cập nhật chỉ số đồng hồ thành công!"
      );
    } catch (error) {
  ...
  const calculateAttendanceDays = async () => {
    ...
    if (
      ...
      !(
        userRole === "admin" ||
        userRole === "developer" ||
        userId === "BJHeKQkyE9VhWCpMfaONEf2N28H2"
      )
    ) {
      setAuthError("Hệ thống chưa sẵn sàng hoặc bạn không có quyền.");
      return;
    ...
    if (totalCost <= 0) {
      setBillingError(
        "Vui lòng tính toán tổng chi phí điện nước trước khi chia tiền."
      );
      return;
    }
    ...
    try {
      ...
      querySnapshot.forEach((docSnap) => {
        ...
        if (
          allMembersToCalculate.some(
            (member) => member.id === data.residentId
          ) &&
          data.status === 1
        ) {
      ...
      if (totalDaysAcrossAllResidentsLocal > 0 && totalCost > 0) {
        calculatedCostPerDayLocal =
          totalCost / totalDaysAcrossAllResidentsLocal;
      ...
      // Cập nhật quỹ phòng
      const currentMonthSurplusOrDeficit =
        totalCost - totalRoundedIndividualCosts;
      const previousRemainingFund =
        costSharingHistory.length > 0
          ? costSharingHistory[0].remainingFund || 0
          : 0;
      calculatedRemainingFund =
        previousRemainingFund + currentMonthSurplusOrDeficit;
      ...
      const newCostSharingDocRef = await addDoc(
        costSharingHistoryCollectionRef,
        {
      ...
      for (const member of allMembersToCalculate) {
        const userLinkedToResident = allUsersData.find(
          (user) => user.linkedResidentId === member.id
        );
        if (userLinkedToResident) {
          const cost = individualCalculatedCostsLocal[member.id]?.cost || 0;
          const message = `Bạn có hóa đơn tiền điện nước cần đóng ${cost.toLocaleString(
            "vi-VN"
          )} VND cho kỳ từ ${startDate} đến ${endDate}.`;
          await createNotification(
  ...
  const generatePaymentReminder = async () => {
    ...
    if (
      totalCost === 0 ||
      totalCalculatedDaysAllResidents === 0 ||
      Object.keys(individualCosts).length === 0
    ) {
      setGeneratedReminder(
        "Vui lòng tính toán chi phí điện nước và ngày có mặt trước."
      );
      return;
    }
    ...
    const residentName = residents.find(
      (r) => r.id === selectedResidentForReminder
    )?.name;
...
Số tiền ${residentName} cần đóng là ${individualCosts[
      selectedResidentForReminder
    ]?.cost.toLocaleString("vi-VN")} VND cho kỳ từ ${period}.
...
Tin nhắn nên ngắn gọn, thân thiện và rõ ràng.`;
    ...
    try {
      ...
      setGeneratedReminder(
        "Lỗi khi kết nối với Gemini API. Vui lòng kiểm tra kết nối mạng."
      );
  ...
  const handleToggleBillPaidStatus = async (billId, currentStatus) => {
    ...
    if (
      ...
      setAuthError(
        "Bạn không có quyền hoặc không có hồ sơ cư dân liên kết để thực hiện thao tác này."
      );
      return;
    }
  ...
  const handleAddCleaningTask = async () => {
    ...
    if (
      newCleaningTaskName.trim() === "" ||
      !newCleaningTaskDate ||
      !selectedResidentForCleaning
    ) {
      setAuthError(
        "Vui lòng nhập đầy đủ thông tin công việc, ngày và người thực hiện."
      );
      return;
    }
    ...
    const assignedResident = residents.find(
      (res) => res.id === selectedResidentForCleaning
    );
    ...
    try {
      await addDoc(cleaningTasksCollectionRef, {
        ...
        assignedToResidentName: assignedResident
  ...
  const handleToggleIndividualPaymentStatus = async (
    costSharingId,
    residentId,
    currentStatus
  ) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        const updatedIndividualCosts = JSON.parse(
          JSON.stringify(currentData.individualCosts || {})
        );
        ...
        setSelectedCostSharingDetails((prevDetails) => {
          if (!prevDetails || prevDetails.id !== costSharingId)
            return prevDetails;
          return {
            ...prevDetails,
            individualCosts: updatedIndividualCosts,
      ...
      setAuthError(
        `Lỗi khi cập nhật trạng thái thanh toán cá nhân: ${error.message}`
      );
    }
  };
  ...
  const handleAssignShoeRack = async () => {
    ...
    const shoeRackDocRef = doc(
      db,
      `artifacts/${currentAppId}/public/data/shoeRackAssignments`,
      selectedShelfNumber
    );
    const assignedResident = residents.find(
      (res) => res.id === selectedResidentForShelf
    );
    ...
    if (
      existingAssignment &&
      existingAssignment.residentId !== selectedResidentForShelf
    ) {
      console.warn(
        `Tầng kệ ${selectedShelfNumber} đã được gán cho ${existingAssignment.residentName}. Sẽ ghi đè.`
      );
    }
    // Kiểm tra xem cư dân đã được gán cho kệ khác chưa
    const existingResidentAssignment = Object.entries(shoeRackAssignments).find(
      ([shelf, assignment]) =>
        assignment.residentId === selectedResidentForShelf &&
        shelf !== selectedShelfNumber
    );
    if (existingResidentAssignment) {
      console.warn(
        `${assignedResident.name} đã được gán cho tầng kệ ${existingResidentAssignment[0]}. Sẽ di chuyển.`
      );
      // Xóa gán cũ
    ...
    try {
      ...
      console.log(
        `Đã gán ${
          assignedResident ? assignedResident.name : "Unknown"
        } vào tầng kệ ${selectedShelfNumber}.`
      );
    } catch (error) {
  ...
  const handleGenerateCleaningSchedule = async () => {
    ...
    if (
      ...
      setAuthError(
        "Vui lòng đăng nhập hoặc bạn không có quyền để tạo lịch tự động."
      );
      return;
    }
    ...
    if (activeResidents.length === 0) {
      setAuthError(
        "Vui lòng thêm ít nhất một người đang hoạt động vào danh sách để tạo lịch."
      );
      return;
    }
    ...
    endDateForPrompt.setDate(
      today.getDate() + parseInt(numDaysForSchedule) - 1
    );
    ...
    Lịch trình nên kéo dài trong ${numDaysForSchedule} ngày, bắt đầu từ hôm nay (${formatDate(
      today
    )}).
    ...
    try {
      ...
      if (
        ...
        setAuthError(
          "Không thể tạo lịch trực. Vui lòng thử lại hoặc điều chỉnh yêu cầu."
        );
        console.error("Cấu trúc phản hồi Gemini API không mong muốn:", result);
      }
    } catch (error) {
      setAuthError(
        "Lỗi khi kết nối với Gemini API. Vui lòng kiểm tra kết nối mạng hoặc API Key."
      );
      console.error("Lỗi khi gọi Gemini API:", error);
    } finally {
      setIsGeneratingSchedule(false);
  ...
  const handleSaveGeneratedTasks = async () => {
    ...
    if (
      ...
      setAuthError(
        "Vui lòng đăng nhập hoặc bạn không có quyền để lưu lịch trực."
      );
      return;
    }
  ...
  const handleMarkMyPaymentAsPaid = async () => {
    ...
    const updatedIndividualCosts = JSON.parse(
      JSON.stringify(latestCostSharingRecord.individualCosts)
    );

    // Đảm bảo chỉ cập nhật trạng thái của người dùng hiện tại
    if (updatedIndividualCosts[loggedInResidentProfile.id]) {
      if (
        typeof updatedIndividualCosts[loggedInResidentProfile.id] === "number"
      ) {
    ...
    try {
      await updateDoc(costSharingDocRef, {
        individualCosts: updatedIndividualCosts,
      });
      console.log(
  ...
  const [showFinalizeDepartureModal, setShowFinalizeDepartureModal] =
    useState(false);
      ...
      return (
        <div className="p-6 bg-red-50 dark:bg-gray-700 rounded-2xl shadow-lg max-w-5xl mx-auto text-center">
          ...
          <p className="text-gray-700 dark:text-gray-300 mb-4">
            Vui lòng vào mục "Chỉnh sửa thông tin cá nhân" để thêm và xác minh
    ...
    if (userRole === "admin" || userId === "BJHeKQkyE9VhWCpMfaONEf2N28H2") {
      switch (activeSection) {
                  ...
                  <div className="mb-4">
                    <label className="inline-flex items-center">
                      <input
                        ...
                        onChange={(e) =>
                          setShowInactiveResidents(e.target.checked)
                        }
                      />
                      <span className="ml-2 text-gray-700 dark:text-gray-300">
                        Hiển thị người đã vô hiệu hóa
                      </span>
                    </label>
                  </div>
                      ...
                      return (
                        <li key={resident.id} className="...">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              onClick={() =>
                                handleToggleResidentActiveStatus(
                                  resident.id,
                                  resident.name,
                                  isActive
                                )
        ...
        case "attendanceTracking": {
          // Bọc trong {} để có thể khai báo biến
                        ...
                        return (
                          ...
                          >
                            ...
                            {Array.from(
                              ...
                              const isPresent =
                                monthlyAttendanceData[resident.id]?.[
                                  dayString
                                ] === 1;
                              return (
                                <td
            ...
            <div className="p-4 md:p-6 bg-yellow-50 dark:bg-gray-700 rounded-2xl shadow-lg w-full">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
                ...
                <button
                  ...
                  className="mt-3 sm:mt-0 px-4 py-2 bg-purple-600 text-white font-semibold rounded-xl shadow-md hover:bg-purple-700"
                >
          ...
          return (
            <div className="p-4 md:p-6 bg-orange-50 dark:bg-gray-700 rounded-2xl shadow-lg w-full">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
                ...
                <button
                  onClick={() => setShowCostSharingHistoryModal(true)}
                  className="mt-3 sm:mt-0 px-4 py-2 bg-purple-600 text-white font-semibold rounded-xl shadow-md hover:bg-purple-700"
                >
                  <i className="fas fa-history mr-2"></i>
                      ...
                      {costSharingHistory.map((summary) => (
                        ...
                        >
                          ...
                          <td className="py-3 px-6 text-right whitespace-nowrap">
                            <span
                              className={`font-bold ${
                                summary.remainingFund >= 0
                                  ? "text-green-700 dark:text-green-300"
                                  : "text-red-700 dark:text-red-300"
                              }`}
                            >
                              {summary.remainingFund?.toLocaleString("vi-VN")}{" "}
                              VND
                            </span>
                          </td>
                          <td className="py-3 px-6 text-center">
                            <button
                              onClick={() =>
                                setSelectedCostSharingDetails(summary)
                              }
          ...
          return (
            <div className="p-6 bg-purple-50 dark:bg-gray-700 rounded-2xl shadow-lg mt-8 max-w-5xl mx-auto">
              ...
              <button
                ...
                disabled={
                  residents.filter((res) => res.isActive !== false).length === 0
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-2">
                          <input
                            ...
                            onChange={() =>
                              handleToggleCleaningTaskCompletion(
                                task.id,
                          ...
                          <button
                            onClick={() =>
                              handleDeleteCleaningTask(task.id, task.name)
                            }
                  ...
                  <ul className="space-y-3">
                    {[...Array(8)].map((_, i) => {
                      ...
                      const isMyShelf =
                        loggedInResidentProfile &&
                        assignment &&
                        assignment.residentId === loggedInResidentProfile.id;
                      return (
                        <li
                          key={shelfNum}
                          className={`flex items-center justify-between p-3 rounded-lg shadow-sm border ${
                            isMyShelf
                              ? "bg-yellow-200 dark:bg-yellow-900 border-yellow-400"
                          ...
                          {assignment ? (
                            ...
                            >
                              ...
                              {userRole === "admin" && ( // Chỉ hiển thị nút xóa cho admin
                                <button
                                  onClick={() =>
                                    handleClearShoeRackAssignment(shelfNum)
                                  }
        ...
        case "commonRoomInfo": {
          // 1. Tạo một danh sách đã được lọc để sử dụng chung cho cả 2 giao diện
          ...
          // Phân quyền hiển thị
          if (userRole === "admin" || userRole === "developer") {
            // ===== GIAO DIỆN "THẺ THÀNH VIÊN" CHO ADMIN & DEVELOPER =====
            return (
              <div className="p-4 md:p-6 bg-blue-50 dark:bg-gray-700 rounded-2xl shadow-lg w-full">
                ...
                {membersToDisplay.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {/* 2. Sử dụng danh sách đã lọc ở đây */}
                              ...
                              <span
                                className={`absolute bottom-0 right-0 block h-4 w-4 rounded-full border-2 border-white dark:border-gray-800 ${
                                  resident.status === "active"
                                    ? "bg-green-500"
                                    : resident.status === "pending_departure"
                                    ? "bg-yellow-500"
                                    : "bg-gray-400"
                                }`}
                                title={
                        ...
                        {membersToDisplay.map((resident) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="py-3 px-4 whitespace-nowrap">
                                {linkedUser?.email || "N/A"}
                              </td>
                              <td className="py-3 px-4 whitespace-nowrap">
                                {linkedUser?.phoneNumber || "N/A"}
                              </td>
                              <td className="py-3 px-4 whitespace-nowrap">
                                {linkedUser?.studentId || "N/A"}
                              </td>
                            </tr>
                          );
                        })}
            ...
            <div className="p-6 bg-yellow-50 dark:bg-gray-700 rounded-2xl shadow-lg max-w-5xl mx-auto">
                          ...
                          <p className="text-gray-600 dark:text-gray-400 text-sm mb-3">
                            ...
                            <span className="font-medium">
                              {memory.uploadedByName ||
                                allUsersData.find(
                                  (u) => u.id === memory.uploadedBy
                                )?.fullName ||
                                "Người dùng ẩn danh"}
                            </span>
                          </p>
                          {(userRole === "admin" ||
                            userId === memory.uploadedBy) && (
              ...
              {totalMemoriesCount > itemsPerPageMemories && (
                <div className="flex justify-center items-center space-x-4 mt-8">
                  <button
                    onClick={() =>
                      setCurrentPageMemories((prev) => Math.max(1, prev - 1))
                    }
                  ...
                  <button
                    onClick={() =>
                      setCurrentPageMemories((prev) =>
                        Math.min(totalPagesMemories, prev + 1)
                      )
                    }
                    ...
                    <form onSubmit={handleAddMemory} className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewMemoryEventName(e.target.value)
                          }
                          className="shadow-sm appearance-none border border-gray-300 dark:border-gray-600 rounded-xl w-full py-2 px-4 text-gray-700 dark:text-gray-300 leading-tight focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent bg-white dark:bg-gray-700"
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewMemoryPhotoDate(e.target.value)
                          }
                          className="shadow-sm appearance-none border border-gray-300 dark:border-gray-600 rounded-xl w-full py-2 px-4 text-gray-700 dark:text-gray-300 leading-tight focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent bg-white dark:bg-gray-700"
                          required
                        />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          multiple
                          onChange={(e) =>
                            setNewMemoryImageFile(Array.from(e.target.files))
                          }
        ...
        case "formerResidents":
          const filteredFormerResidents = formerResidents.filter(
            (resident) =>
              resident.name
                ?.toLowerCase()
                .includes(searchTermFormerResident.toLowerCase()) ||
              resident.studentId
                ?.toLowerCase()
                .includes(searchTermFormerResident.toLowerCase())
          );
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          MSSV: {resident.studentId}
                        </p>
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          Liên hệ: {resident.contact}
                        </p>
                        {resident.notes && (
                          <p className="text-xs text-gray-500 dark:text-gray-300 mt-1">
                            Ghi chú: {resident.notes}
                          </p>
                        )}
                      </div>
                    </div>
                    {userRole === "admin" && (
                      <div className="flex space-x-2">
                        ...
                        <button
                          onClick={() =>
                            handleDeleteFormerResident(resident.id)
                          }
                    ...
                    >
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewFormerResidentName(e.target.value)
                          }
                      ...
                      <div>
                        <label
                          htmlFor="formerResidentStudentId"
                          className="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2"
                        >
                          Mã số sinh viên:
                        </label>
                        <input
                          ...
                          onChange={(e) =>
                            setNewFormerResidentStudentId(e.target.value)
                          }
                          className="shadow-sm appearance-none border border-gray-300 dark:border-gray-600 rounded-xl w-full py-2 px-4 text-gray-700 dark:text-gray-300 leading-tight focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white dark:bg-gray-700"
                          required
                        />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewFormerResidentContact(e.target.value)
                          }
                          className="shadow-sm appearance-none border border-gray-300 dark:border-gray-600 rounded-xl w-full py-2 px-4 text-gray-700 dark:text-gray-300 leading-tight focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white dark:bg-gray-700"
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setNewFormerResidentNotes(e.target.value)
                          }
                          className="shadow-sm appearance-none border border-gray-300 dark:border-gray-600 rounded-xl w-full py-2 px-4 text-gray-700 dark:text-gray-300 leading-tight focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white dark:bg-gray-700"
                          rows="3"
                      ...
                      <div>
                        ...
                        <input
                          ...
                          accept="image/*"
                          onChange={(e) =>
                            setNewFormerResidentAvatarFile(
                        ...
                        {isUploadingFormerResidentAvatar && (
                          <div className="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700 mt-2">
                            <div
                              ...
                              style={{
                                width: `${uploadFormerResidentAvatarProgress}%`,
                              }}
                            ></div>
                          </div>
                        )}
                        {uploadFormerResidentAvatarProgress > 0 &&
                          uploadFormerResidentAvatarProgress < 100 && (
                            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1 text-right">
                              {uploadFormerResidentAvatarProgress}% tải lên
                            </p>
                          )}
                      </div>
                      <div className="flex space-x-4 mt-6">
                ...
                <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 p-4">
                  <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-2xl w-full max-w-md">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 text-center mb-4">
                      Chỉ những thành viên đã được "Vô hiệu hóa" ở mục Quản lý
                      người ở mới xuất hiện trong danh sách này.
                    </p>
                    <div className="space-y-4">
                      <div>
                        <label
                          htmlFor="residentToMove"
                          className="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2"
                        >
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setSelectedResidentToMove(e.target.value)
                          }
                    ...
                    >
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setNewNotificationRecipient(e.target.value)
                          }
                          className="shadow-sm appearance-none border rounded-xl w-full py-2 px-3 text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 focus:ring-blue-500 focus:border-blue-500"
                        >
              ...
              {Object.keys(monthlyConsumptionStats).length === 0 ? (
                ...
                <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
                  <table className="min-w-full bg-white dark:bg-gray-800">
                    ...
                    <tbody className="text-gray-700 dark:text-gray-300 text-sm font-light">
                      {Object.entries(monthlyConsumptionStats).map(
                        ([month, stats]) => (
                          <tr
                            key={month}
                            className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                ...
                {loginHistory.length > 0 ? (
                  loginHistory.map((log) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        Thời gian:{" "}
                        {log.loginAt?.toDate().toLocaleString("vi-VN")}
                      </p>
                      <p
          ...
          return (
            <div className="p-6 bg-cyan-50 dark:bg-gray-700 rounded-2xl shadow-lg max-w-5xl mx-auto">
              <h2 className="text-2xl font-bold text-cyan-800 dark:text-cyan-200 mb-5">
                Quản lý Thành viên Chờ & Tạm thời
              </h2>
              <div className="flex flex-col sm:flex-row gap-3 mb-4">
                <input
                  type="text"
                ...
                />
                {/* Lựa chọn loại thành viên */}
                <select
                  value={newPendingResidentType}
                  onChange={(e) => setNewPendingResidentType(e.target.value)}
                  className="shadow-sm border rounded-xl py-2 px-4"
                >
                  <option value="pending">Thành viên chờ</option>
                  <option value="temporary">Thành viên tạm thời</option>
                </select>
                <button
                  ...
                  >
                    <div>
                      ...
                      <span className="text-xs text-gray-500 ml-2">
                        ({pending.type === "temporary" ? "Tạm thời" : "Chờ"})
                      </span>
                    </div>
                    <button
    ...
    if (userRole === "member") {
      ...
      const myLatestCost =
        costSharingHistory.length > 0 && loggedInResidentProfile
          ? costSharingHistory[0].individualCosts?.[loggedInResidentProfile.id]
              ?.cost || 0
          : 0;
      ...
      switch (activeSection) {
            ...
            <div className="p-6 bg-gray-50 dark:bg-gray-700 rounded-2xl shadow-lg max-w-5xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md flex flex-col items-center justify-center">
                  ...
                  <p
                    className={`text-3xl font-bold ${
                      myLatestCostIsPaid ? "text-green-600" : "text-red-500"
                    } dark:text-green-300`}
                  >
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    {myLatestCostIsPaid
                      ? "Đã đóng"
                      : `Chưa đóng (Kỳ: ${myLatestCostPeriod})`}
                  </p>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md col-span-full">
                  ...
                  {upcomingMyCleaningTasks.length > 0 ? (
                    ...
                    <p className="text-gray-600 dark:text-gray-400 italic">
                      Bạn không có nhiệm vụ trực phòng sắp tới.
                    </p>
                  )}
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md col-span-full">
                  ...
                  {loggedInResidentProfile &&
                  monthlyAttendanceData[loggedInResidentProfile.id] ? (
        ...
        case "attendanceTracking": {
          const currentUserData = allUsersData.find((u) => u.id === userId);
          const memberCanTakeAttendance =
            currentUserData?.canTakeAttendance === true;
          return (
                              ...
                              return (
                                ...
                                >
                                  <input
                                    ...
                                    disabled={
                                      !isMyRow && !memberCanTakeAttendance
                                    }
        ...
        case "memberCostSummary":
          ...
          return (
            <div className="p-6 bg-orange-50 dark:bg-gray-700 rounded-2xl shadow-lg max-w-5xl mx-auto">
              ...
              {!loggedInResidentProfile ? (
                <p className="text-gray-600 dark:text-gray-400 italic text-center py-4">
                  Bạn chưa được liên kết với hồ sơ người ở. Vui lòng liên hệ
                  quản trị viên.
                </p>
              ) : !latestCostSharingRecord ||
                !latestCostSharingRecord.individualCosts ||
                !latestCostSharingRecord.individualCosts[
                  loggedInResidentProfile.id
                ] ? (
                <p className="text-gray-600 dark:text-gray-400 italic text-center py-4">
                ...
                <div>
                  ...
                  <div className="bg-orange-100 dark:bg-gray-800 p-6 rounded-xl shadow-inner text-lg font-semibold text-orange-900 dark:text-orange-100 border border-orange-200 dark:border-gray-600">
                    ...
                    <p className="mb-2">
                      ...
                      {latestCostSharingRecord.periodStart} đến{" "}
                    ...
                    {!latestCostSharingRecord.individualCosts[
                      ...
                      >
                        <i className="fas fa-check-circle mr-2"></i> Đánh dấu đã
                        đóng
                      </button>
                    )}
                  </div>
                    ...
                    loggedInResidentProfile && (
                      ...
                      >
                        {(() => {
                          ...
                          const billPeriod =
                            latestCostSharingRecord.periodStart ||
                            new Date().toISOString().slice(0, 7);
                          const billMonth = billPeriod.split("-")[1];
                          const billYear = billPeriod.split("-")[0];
                          const description = `CK ${fullName
                            .split(" ")
                            .slice(-1)
                            .join("")} KTX T${billMonth}-${billYear}`;
                          const vietQR_imageUrl = `https://img.vietqr.io/image/${bankId}-${accountNumber}-print.png?amount=${amountToPay}&addInfo=${encodeURIComponent(
                            description
                      ...
                      {residents.map((resident) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-3 px-4 whitespace-nowrap">
                              {linkedUser?.birthday || "N/A"}
                            </td>
                            <td className="py-3 px-4 whitespace-nowrap">
                              {linkedUser?.dormEntryDate || "N/A"}
                            </td>
                            <td className="py-3 px-4 whitespace-nowrap">
                              {linkedUser?.academicLevel || "N/A"}
                            </td>
                            <td className="py-3 px-4 whitespace-nowrap">
                              <span
                  ...
                  {formerResidents.map((resident) => (
                    ...
                    >
                      ...
                      <div className="p-4">
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-300">
                          <i className="fas fa-phone mr-2"></i>SĐT:{" "}
                          {resident.phoneNumber || "N/A"}
                        </p>
                        <p className="text-sm text-gray-600 dark:text-gray-300">
                          <i className="fas fa-id-badge mr-2"></i>MSSV:{" "}
                          {resident.studentId || "N/A"}
                        </p>
                        <p className="text-sm text-gray-600 dark:text-gray-300">
                          <i className="fas fa-birthday-cake mr-2"></i>Ngày
                          sinh: {resident.birthday || "N/A"}
                        </p>
                        <p className="text-sm text-gray-600 dark:text-gray-300">
                          <i className="fas fa-calendar-alt mr-2"></i>Ngày nhập
                          KTX: {resident.dormEntryDate || "N/A"}
                        </p>
                        <p className="text-sm text-gray-600 dark:text-gray-300">
                          <i className="fas fa-graduation-cap mr-2"></i>Cấp:{" "}
                          {resident.academicLevel || "N/A"}
                        </p>
                        <p className="text-sm text-gray-600 dark:text-gray-300">
                          <i className="fas fa-door-open mr-2"></i>Ngày ra khỏi
                          phòng:{" "}
                          {resident.deactivatedAt &&
                          typeof resident.deactivatedAt.toLocaleDateString ===
                            "function"
        ...
        case "feedback":
          if (userRole === "member") {
            return (
              <div className="p-6 bg-gray-50 dark:bg-gray-700 rounded-2xl shadow-lg max-w-5xl mx-auto">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-5">
                  Gửi góp ý cho chúng mình
                </h2>
                <p className="text-gray-600 dark:text-gray-400 mb-4">
                  Mọi ý kiến đóng góp của bạn đều rất quý giá để giúp ứng dụng
                  ngày càng tốt hơn. Cảm ơn bạn!
                </p>
